...
def get_cubes(mw3d, data_dir, output_dir, crop_size, cube_size, noise_scale,noise_mean, inp):
    '''
...
def get_cubes_list(mw3d, mrc_list, data_dir,output_dir, crop_size, cube_size, batch_size=8, noise_scale = 1, noise_mean = 0):
    '''
    ...
    if preprocessing_ncpus > 1:
        func = partial(get_cubes, mw3d, data_dir, output_dir, crop_size, cube_size,noise_scale, noise_mean)
...
def map_refine(halfmap, mask, fsc3d, voxel_size, limit_res, output_dir = "results", output_base="half1", n_subvolume = 50, cube_size = 64, crop_size = 96, weighting=False):
    ...
    mkfolder(data_dir)
    fsc3d = fsc3d.copy()
    fsc3d = skimage.transform.resize(fsc3d, [384,384,384], order = 0)
    ...
    halfmap = fsc_filter(halfmap, fsc3d,limit_r)
