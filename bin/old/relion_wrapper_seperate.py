...
if __name__=="__main__":  
    ...
    if (healpix < limit_healpix):     
        ...
        for i in range (1,15):
            try:
                with mrcfile.open(mrc1) as f2:
                    pass#f2.data.astype(np.float32).copy() 
            except:
                print("Waiting for half1")
                time.sleep(30)
        ...
        if (check is True):                
            ...
            if half_str == "half1":
                execute_deep(mrc1, dir, basename, var, "0,1", epochs = 0, mask_file = mask_file, pretrained_model = model1, batch_size = batch_size, acc_batches=acc_batches, alpha=alpha)
                shutil.move(out_mrc1, '%s/prepredicted_%s_it%s_half1_class001_external_reconstruct.mrc' %(dir,basename,var))
                execute_deep(mrc1, dir, basename, var, "0,1", epochs = epochs, mask_file = mask_file, pretrained_model = model1, batch_size = batch_size, acc_batches=acc_batches, alpha=alpha)
                shutil.move(mrc1, '%s/precorrect_%s_it%s_half1_class001_external_reconstruct.mrc' %(dir,basename,var))
            ...
            if half_str == "half2":
                execute_deep(mrc2, dir, basename, var, "2,3", epochs = 0, mask_file = mask_file, pretrained_model = model2, batch_size = batch_size, acc_batches=acc_batches, alpha=alpha)
                shutil.move(out_mrc2, '%s/prepredicted_%s_it%s_half2_class001_external_reconstruct.mrc' %(dir,basename,var))
                execute_deep(mrc2, dir, basename, var, "2,3", epochs = epochs, mask_file = mask_file, pretrained_model = model2, batch_size = batch_size, acc_batches=acc_batches, alpha=alpha)
                shutil.move(mrc2, '%s/precorrect_%s_it%s_half2_class001_external_reconstruct.mrc' %(dir,basename,var))   
