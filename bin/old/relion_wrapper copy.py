...
if __name__=="__main__":  
    ...
    if (healpix < 2):     
        ...
        if ((check_final is True) or (check is True)) and (half_str == 'half1' or half_str == 'class001'):                
            ...
            execute_deep(mrc1, dir, basename, var, gpu, epochs = 0, mask_file = mask_file, pretrained_model = model1)
            execute_deep(mrc2, dir, basename, var, gpu, epochs = 0, mask_file = mask_file, pretrained_model = model2)
            ...
            if check is True:
                ...
                shutil.move(out_mrc1, '%s/prepredicted_%s_it%s_half1_class001_unfil.mrc' %(dir,basename,var))
                shutil.move(out_mrc2, '%s/prepredicted_%s_it%s_half2_class001_unfil.mrc' %(dir,basename,var))                
            ...
            if check is True:
                ...
                shutil.move(mrc1, '%s/precorrect_%s_it%s_half1_class001_unfil.mrc' %(dir,basename,var))
                shutil.move(mrc2, '%s/precorrect_%s_it%s_half2_class001_unfil.mrc' %(dir,basename,var))     
