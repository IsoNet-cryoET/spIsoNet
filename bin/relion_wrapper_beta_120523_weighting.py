...
def angular_whitening(in_name,out_name,resolution_initial, limit_resolution, cone_angle=10):
    import mrcfile
    ...
    half_angle_rad = np.radians(cone_angle)
...
if __name__=="__main__":  
    ...
    if (check_final is True):
        ...
        #angular_whitening
        if (healpix >= limit_healpix) and (limit_resolution < 15):
            mrc_angwhiten_backup = '%s/%s_it%s_%s_class001_unfil_angwhiten_backup.mrc' %(dir,basename,var,half_str)
            angular_whitening(mrc_unfil,mrc_unfil,resolution_initial, limit_resolution, cone_angle=10) 
            shutil.copy(mrc_unfil, mrc_angwhiten_backup)
            shutil.copy(mrc_unfil, mrc_overwrite)
