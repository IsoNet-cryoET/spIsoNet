...
def get_cubes(mw3d, result_dir, data_dir, iter_count, crop_size, cube_size,  inp):
    ...
    for i in range(len(rotation_list)): 
        ...
        data_X = data_X*data_mask + data_Y*(1-data_mask)
        data_Y = data_Y*data_mask + np.average(data_Y[(1-data_mask)>0.5]) * (1-data_mask)# + data_Y*(1-data_mask)# + 0.5*data_Y*(1-data_mask)
...
def map_refine(halfmap, mask, fsc3d, voxel_size, limit_res, output_file="half1.mrc", n_subvolume = 50, cube_size = 64, crop_size = 96, weighting=False):
    log_level = "info"
    if log_level == "debug":
    ...
    for iter_count in range(1,num_iterations+1):        
            ...
            metrics = network.train(data_dir,gpuID=0, batch_size=12, epochs = 1, steps_per_epoch = 200, acc_grad = True) #train based on init model and save new one as model_iter{num_iter}.h5
    ...
    network.predict_map(halfmap, fsc3d, output_file=output_file, voxel_size = voxel_size )
