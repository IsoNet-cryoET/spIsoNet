import logging
from spIsoNet.preprocessing.prepare import get_cubes_list,get_noise_level, prepare_first_iter
from spIsoNet.util.dict2attr import save_args_json,load_args_from_json
import numpy as np
import os
import sys
import shutil
from spIsoNet.util.metadata import MetaData
    ...
    try:
        ...
        for num_iter in range(current_iter,args.iterations + 1):        
            ...
            if num_iter>=args.noise_start_iter[0] and (not os.path.isdir(args.noise_dir) or len(os.listdir(args.noise_dir))< num_noise_volume ):
                from spIsoNet.util.noise_generator import make_noise_folder
